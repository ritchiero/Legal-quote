    ...
    return (
          <div className="h-screen flex flex-col pt-6 px-6 overflow-hidden">
                <div className="flex items-center justify-between mb-5">
                        <div>
                                  <h1 className="text-[22px] font-bold tracking-tight">Pipeline</h1>h1>
                                  <p className="text-sm text-text-secondary mt-0.5">{quotations.length} cotizaciones en total</p>p>
                        </div>div>
                </div>div>
                  ...
                  {grouped.map(col => (
                      <div
                                  ...
                                  <div className="flex items-center gap-2 mb-3 px-1">
                                                ...
                                                <span className="text-[13px] font-semibold">{col.label}</span>span>
                                  ...
                                  <div
                                    ...
                                    {col.items.map(q => (
                                                                  <div
                                                                                    ...
                                                                                    >
                                                                                    <div className="text-[13px] font-semibold leading-snug mb-1.5">{getService(q)}</div>div>
                                                                    {getClient(q) && (
                                                                                                          <div className="text-[11px] text-text-secondary mb-2 flex items-center gap-1">
                                                                                                                                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>svg>
                                                                                                            {getClient(q)}
                                                                                                            </div>div>
                                                                                    )}
                                                                                    <div className="flex items-center justify-between">
                                                                                                        <span className="text-[12px] font-bold">{q.precio || ''}</span>span>
                                                                                                        <span className="text-[10px] text-text-secondary">{fmtDate(q.createdAt)}</span>span>
                                                                                      </div>div>
                                                                  </div>div>
                                                                ))}
                                  </div>div>
